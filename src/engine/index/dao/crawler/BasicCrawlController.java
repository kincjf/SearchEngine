...
/**
 * perform crawler4j configuration, url filtering for seed url
 * @author KIMSEONHO
 */
public class BasicCrawlController {
	/**
	 * 각 url마다 할당될 idxNum (크롤러에 의해 +1씩 증가함)
	 * idxNum = indexing된 url의 갯수 + 1 부터 시작 
	 */
	private static List<Integer> newIdxList;	// 각각 할당될 idxNum (크롤러에 의해 +1씩 증가함)
	
	/**
	 * index되지 않은 url의 idx list
	 */
	private static ArrayList<Integer> updateIdxList;
	
	/**
	 * 존재하지 않은 url에 대한 idx
	 * so crawler thread has processing, new idx must have concurrency
	 * for idx safety, atomicity during in creation data
	 */
	private static ConcurrentHashMap<Integer, IndexDataBean> newIdxMap;
	
	/**
	 * matching update idx and hit times
	 * for updating hit index term 
	 */
	...
	/**
	 * crawler initialize, start
	 * @param list			bookmark data
	 * @param contributor			who is upload bookmark data?
	 * @param crawlStorageFolder			where is location for save file 
	 * @param numberOfCrawlers				thread number for url crawling
	 */
	...
	public void start(JSONArray list, String contributor, String crawlStorageFolder, int numberOfCrawlers) {
		...
		// db 접근이 많기 때문에 부하를 줄이기 위해서 close는 직접 한다.
		...
		// until bookmark data is exist in list
		while(iterator.hasNext()) {
			...
			if(urlList.contains(url)) {		// 이미 url이 등록이 되어있는 경우
				...
				if(isSuccess) {		// 등록이 되어있지 않은 경우 hit수 갱신
					hit = indexDAO.updateURLHit(url);		// ((기존 hit 수) + 1) - 갱신
				}
